...
def feature_engineering_from_file(in_path: str,
    ...
    # 主键是重要数据：缺失直接删
    df = df.dropna(subset=[id_col, ts_col]).copy()
    ...
    alpha_feats = [c2 for c2 in df.columns if any(c2.endswith(f"_z{w}") for w in z_windows)]
    ...
    # 非重要特征缺失：不删行
    X_alpha = df[base_cols + alpha_feats].copy()
    X_risk  = df[base_cols + risk_candidates].copy()
    factor_pool = X_alpha.copy()

    # regime 特征是重要数据：用于聚类，缺失就删
    X_regime = (
        df[base_cols + regime_core].dropna(subset=regime_core)
        if regime_core else df[base_cols].iloc[0:0]
    )

    # 标签是重要数据：缺失就删
    y_df = df[base_cols + ["y_ret_k","y_up_k","y_vol_k"]].dropna(
        subset=["y_ret_k","y_up_k","y_vol_k"]
    )
    ...
    return out_path, {
        ...
        "y_ret_k":  y_df[base_cols + ["y_ret_k","y_up_k"]].reset_index(drop=True),
        "y_vol_k":  y_df[base_cols + ["y_vol_k"]].reset_index(drop=True),
    }
...
def train_gbdt_alpha(X_alpha: pd.DataFrame, y_ret: pd.DataFrame,
    ...
    base_cols = [id_col, ts_col]
    df = X_alpha.merge(y_ret[base_cols+["y_ret_k"]], on=base_cols, how="inner") \
                .dropna(subset=["y_ret_k"]) \
                .sort_values([ts_col, id_col])
...
def lasso_select(factor_pool: pd.DataFrame, y_ret: pd.DataFrame,
    ...
    base_cols = [id_col, ts_col]
    df = factor_pool.merge(y_ret[base_cols+["y_ret_k"]], on=base_cols, how="inner") \
                    .dropna(subset=["y_ret_k"]) \
                    .sort_values([ts_col, id_col])
...
def train_stack_alpha(X_alpha: pd.DataFrame, y_ret: pd.DataFrame,
    ...
    stack = StackingRegressor(estimators=base_estimators, final_estimator=meta,
...
def train_risk_model(X_risk: pd.DataFrame, y_vol: pd.DataFrame,
    ...
    base_cols = [id_col, ts_col]
    df = X_risk.merge(y_vol[base_cols+["y_vol_k"]], on=base_cols, how="inner") \
               .dropna(subset=["y_vol_k"]) \
               .sort_values([ts_col, id_col])
...
if __name__ == "__main__":
    ...
    out_csv, res = feature_engineering_from_file(
        in_path,
        out_dir="./outputs",
    ...
    y_ret_df = train_df[["ticker","trade_date","y_ret_k"]].dropna(subset=["y_ret_k"])
    y_vol_df = train_df[["ticker","trade_date","y_vol_k"]].dropna(subset=["y_vol_k"])

    # —— LASSO 选因 ——
    selected = lasso_select(
        res["factor_pool"], y_ret_df,
        out_path="./outputs/lasso_selected_features.txt"
    )

    # —— Return Prediction：GBDT ——
    _gbdt_scores = train_gbdt_alpha(
        res["X_alpha"], y_ret_df,
        out_scores="./outputs/gbdt_scores.csv"
    )

    # —— Signal Integration：Stacking ——
    _stack_scores = train_stack_alpha(
        res["X_alpha"], y_ret_df,
